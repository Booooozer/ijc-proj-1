# 1 "primes.c"
# 1 "/home/tom/school/semester 2/ijc/proj1//"
# 1 "<built-in>"
# 1 "<command-line>"
...
# 63 "bitset.h"
    ...
    inline void bitset_setbit(bitset_t jmeno_pole, unsigned long index, char vyraz) {
        (vyraz == 1) ? (jmeno_pole[(index / (sizeof(unsigned long) * 8)) + 1] |= (1UL << ((index) % (sizeof(unsigned long) * 8)))) :
    ...
    inline unsigned long bitset_getbit(bitset_t jmeno_pole, unsigned long index) {
        return (jmeno_pole[(index / (sizeof(unsigned long) * 8)) + 1] & (1UL << ((index) % (sizeof(unsigned long) * 8))));
    }
# 6 "primes.c" 2
# 1 "eratosthenes.h" 1
...
# 7 "/usr/include/bits/types/clock_t.h" 3 4
...
int main() {
    ...
    _Static_assert(200000000 > 0, "Velikost pole musi byt vetsi nez 0"); unsigned long p [((200000000 / (sizeof(unsigned long) * 8)) + ((200000000 % (sizeof(unsigned long) * 8)) ? 1 : 0) + 1)] = {200000000};
    Eratosthenes(p);

    int toRead = 0;
    unsigned long primes[10];

    for (bitset_index_t i = 200000000 - 1; toRead < 10; i--) {
